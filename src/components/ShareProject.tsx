...
export default function ShareProject({
  ...
  return (
    <div className="bg-white min-h-[400px] p-4 rounded-xl">
      {/* 搜索和排序 */}
      <div className="flex items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-2 text-sm">
          <span>{lang === 'zh' ? '市值' : 'Market Cap'}</span>
          <button onClick={handleSort} className="flex flex-col gap-[1px]">
            <ChevronUp
              className={`w-3 h-3 ${sort === 'asc' ? 'text-black' : 'text-gray-400'}`}
            />
            <ChevronDown
              className={`w-3 h-3 -mt-1 ${sort === 'desc' ? 'text-black' : 'text-gray-400'}`}
            />
          </button>
        </div>
        <input
              ...
              <div className="flex items-start justify-between mb-2">
                <div>
                  ...
                  <div className="text-gray-500 text-xs mt-1">
                    ...
                    {(Number(item.total_supply) * (item.lastPrice || 0)).toFixed(0)}
                  </div>
                </div>
